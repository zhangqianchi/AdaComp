...
if __name__ == '__main__':
    ...
    for lora_index, lora_model_path in enumerate(lora_model_paths):
        ...
        tokenizer = CodeLlamaTokenizer.from_pretrained(lora_model_path)
        print(base_model.get_input_embeddings().weight.size(0))
        ...
        if hasattr(peft.LoraModel, 'merge_and_unload'):
            ...
            for k in lora_keys:
                ...
                base_model_sd[original_key] += (transpose(lora_model_sd[lora_b_key].float() @ lora_model_sd[lora_a_key].float(), fan_in_fan_out) * lora_scaling)
